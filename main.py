...
#Water stage obstacle images
BASE = os.path.dirname(os.path.abspath(__file__))
WATER_IMGS = {}
for fname in ["madeljellyfish.png", "moniquejellyfish.png", "yanahjellyfish.png", "francinejellyfish.png"]:
    key = fname.replace(".png", "")
    try:
        img = pygame.image.load(os.path.join(BASE, fname)).convert_alpha()
        WATER_IMGS[key] = pygame.transform.smoothscale(img, (90, 90))
    except Exception as e:
        print(f"Could not load {fname}: {e}")
try:
    run_img_raw = pygame.image.load(os.path.join(BASE, "linnoellerunrunrun.png")).convert_alpha()
    RUN_IMG = pygame.transform.smoothscale(run_img_raw, (75, 95))
except Exception as e:
    print(f"Could not load linnoellerunrunrun.png: {e}")
    RUN_IMG = None
#Car stage player image
try:
    car_img_raw = pygame.image.load(os.path.join(BASE, "subwaysurfers.png")).convert_alpha()
    CAR_IMG = pygame.transform.smoothscale(car_img_raw, (90, 110))
except Exception as e:
    print(f"Could not load subwaysurfers.png: {e}")
    CAR_IMG = None
#Stage background image
try:
    bg_raw = pygame.image.load(os.path.join(BASE, "background.png")).convert()
    BG_IMG = pygame.transform.smoothscale(bg_raw, (W, H))
except Exception as e:
    print(f"Could not load background.png: {e}")
    BG_IMG = None

class GifSprite:
    def __init__(self, path, w, h, fps=12):
        self.frames = []
        self.fps    = fps
        self._load(path, w, h)
        self.frame_idx = 0
    ...
    def _load(self, path, w, h):
        try:
            ...
            print(f"Opened OK, mode: {img.mode}")
            while True:
                frame = img.copy().convert("RGBA")
                arr = np.array(frame, dtype=np.float32)
                r, g, b, a = arr[...,0], arr[...,1], arr[...,2], arr[...,3]
                #Mask
                mask = (g > 40) & (g > r * 1.1) & (g > b * 1.1)
                arr[..., 3] = np.where(mask, 0, a)
                frame = PIL.Image.fromarray(arr.astype(np.uint8), "RGBA")
                data = frame.tobytes()
...
class Game:
    def __init__(self):
        ...
        BASE = os.path.dirname(os.path.abspath(__file__))
        self.gif_lin = GifSprite(
            os.path.join(BASE, GIF_CONFIG["lin"]["file"]),
            GIF_CONFIG["lin"]["w"], GIF_CONFIG["lin"]["h"]
        )
        self.gif_noelle = GifSprite(
            os.path.join(BASE, GIF_CONFIG["noelle"]["file"]),
            GIF_CONFIG["noelle"]["w"], GIF_CONFIG["noelle"]["h"]
        )
        self.frame        = 0
        self.player_name  = ""
        self.name_cursor  = 0
    ...
    def reset_stage(self):
        ...
        if t == "run":
            self.px, self.py = 100, GROUND_BOTTOM - 44
    ...
    def _spawn_run_section(self, start_x):
        ...
        for _ in range(8):
            ...
            too_close = any(
                o.get("platform") and abs(o["x"] - x) < 80
                for o in self.obstacles
            )
            if too_close:
                continue

            route = random.choice(["top", "top", "bottom"])

            if route == "top":
                pw = random.randint(60, 100)
                toxic = (not last_was_toxic) and random.random() < 0.15
                last_was_toxic = toxic
                color = RED if toxic else BROWN
                self.obstacles.append({
                    "x": x, "y": GROUND_TOP - 18,
                    "w": pw, "h": 18,
                    "color": color, "platform": True, "toxic": toxic
                })
                if random.random() < 0.7:
                    self.coins.append({"x": x + pw // 2, "y": GROUND_TOP - 44, "collected": False})

            else:
                last_was_toxic = False
                toxic_above = any(
                    o.get("toxic") and abs(o["x"] - x) < 120
                    for o in self.obstacles
                )
                #Bottom coin
                if random.random() < 0.5:
                    self.coins.append({"x": x, "y": GROUND_BOTTOM - 20, "collected": False})
                if random.random() < 0.45 and not toxic_above:
                    oh = random.randint(28, 40)
                    self.obstacles.append({
                        "x": x + 30, "y": GROUND_BOTTOM - oh,
                        "w": 28, "h": oh,
                        "color": RED, "platform": False, "toxic": False
                    })        
    ...
    def update_run(self, keys):
        ...
        #Bottom ground landing
        if self.py + 44 >= GROUND_BOTTOM:
            self.py = GROUND_BOTTOM - 44
            self.pvy = 0; self.on_ground = True
            self.drop_through = False; self.jumps_left = 2
        ...
        if not self.drop_through:
            for o in self.obstacles:
                if o.get("platform"):
                    feet = self.py + 44
                    in_x = self.px + 32 > o["x"] and self.px + 4 < o["x"] + o["w"]
                    if in_x and feet >= o["y"] and feet <= o["y"] + 18 and self.pvy >= 0:
                        if o.get("toxic"):
                            self.trigger_gameover(); return
                        self.py = o["y"] - 44
                        self.pvy = 0; self.on_ground = True; self.jumps_left = 2

        if keys[pygame.K_s] and self.on_ground:
            standing_on_platform = any(
                o.get("platform") and
                self.px + 32 > o["x"] and self.px + 4 < o["x"] + o["w"] and
                abs((self.py + 44) - o["y"]) < 4
                for o in self.obstacles
            )
            if standing_on_platform:
                self.drop_through = True; self.on_ground = False; self.pvy = 4

        if self.py > H + 60: self.trigger_gameover(); return

        for o in self.obstacles: o["x"] -= SPEED
        for coin in self.coins:  coin["x"] -= SPEED
        self.obstacles = [o for o in self.obstacles if o["x"] > -160]
    ...
    def update_water(self, keys):
        ...
        if self.frame % 80 == 0:
            kind = random.choice(list(WATER_IMGS.keys())) if WATER_IMGS else "jelly"
            self.obstacles.append({
    ...
    def draw_title(self):
        ...
        for c in self.clouds: c.draw(screen)
         #GIFs
        self.gif_lin.update()
        self.gif_noelle.update()
        self.gif_lin.draw(screen,    GIF_CONFIG["lin"]["x"],    GIF_CONFIG["lin"]["y"])
        self.gif_noelle.draw(screen, GIF_CONFIG["noelle"]["x"], GIF_CONFIG["noelle"]["y"])
    ...
    def draw_run(self):
        ...
        #Player
        if RUN_IMG:
            screen.blit(RUN_IMG, RUN_IMG.get_rect(midbottom=(int(self.px) + 18, int(self.py) + 44)))
        else:
    ...
    def draw_water(self):
        ...
        for o in self.obstacles:
            ...
            img = WATER_IMGS.get(o["kind"])
            if img:
                screen.blit(img, img.get_rect(center=(ox, oy)))
            else:
                pygame.draw.circle(screen, PURPLE, (ox, oy), 22)
    ...
    def draw_car_stage(self):
        ...
        cx,cy=int(self.px),int(self.py)
        if CAR_IMG:
            screen.blit(CAR_IMG, CAR_IMG.get_rect(midbottom=(cx+40, cy+54)))
        else:
            tmp=pygame.Surface((90,60),pygame.SRCALPHA); _draw_car_base(tmp,0,0)
            rotated=pygame.transform.rotate(tmp,-math.degrees(self.car_angle))
            screen.blit(rotated,rotated.get_rect(center=(cx+40,cy+27)).topleft)
    ...
    def draw_name_entry(self):
        ...
        draw_text(screen, "Enter your name for the leaderboard:", font_sm, OFFWHITE, W//2, 200) #Leaderboard input box
